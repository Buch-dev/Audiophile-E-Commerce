...
export const createOrUpdateReview = handleAsyncError(async (req, res, next) => {
  ...
  product.ratings = product.reviews.length > 0 ? (totalRating / product.reviews.length).toFixed(1) : 0;

  await product.save({ validateBeforeSave: false });
  
  return res.status(200).json({
    success: true,
    message: existingReviewIndex !== -1 ? "Review updated successfully" : "Review created successfully",
    product,
  });
});

// Alternative version using route parameter instead of query
export const getProductReviewsByParam = handleAsyncError(async (req, res, next) => {
...
export const deleteReview = handleAsyncError(async (req, res, next) => {
  ...
  const reviews = product.reviews = product.reviews.filter((review) => review._id.toString() !== req.query.id.toString());
  await product.save({ validateBeforeSave: false });
